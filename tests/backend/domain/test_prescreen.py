"""Страж эквивалентности подстрочного пресина в _detect_techs (см. `docs/domain.md`,
«Детекция техов»; объём словаря пришпилен `docs/config.md`).
...
def test_screened_covers_every_tech_of_the_dictionary():
    ...
    все 580 случаев останутся зелёными, а тег перестанет детектиться в проде. Здесь одна
    сторона — литерал: объём словаря стека назван числом в docs/config.md."""
    assert [tech for tech, _rx, _screen in _SCREENED] == list(TECH_PATTERNS)
    assert len(_SCREENED) == 61        # объём словаря стека, docs/config.md
...
def test_prescreen_equivalent_on_real_data():
    # Сильнейший сигнал: ВСЕ реальные тайтлы+сниппеты. Медленный (наив гоняет все 61 regex),
