...
@pytest.mark.parametrize("name", [
    ...
    # resume_profile.json запускающего. Дрейф КОДА ловят стражи дефолтов в test_feed_bridge.py.
    "RESUME_TIERS_PY", "RESUME_ROLE_FIT_PY", "RESUME_CORE_SAT_PY",
    # Язык — ПЕРВАЯ проверка скоринга (14.08.2026). Без неё «Ведущий разработчик 1С»
    # набирал 91 %: три ядровых попадания по базам давали насыщение, а 1С весом 0
    # растворялся в знаменателе. По кешу таких было 498 штук с баллом >= 70.
    "LANG_KEYS_PY", "RESUME_LANGS_PY", "RESUME_LANG_FIT_PY",
