...
#
# НАМЕРЕННОЕ РАСХОЖДЕНИЕ с `experience.py::_GRADE_TO_EXP` (аудит 08.08.2026). Прогон
# «ярлык -> опыт -> грейд» против «то же слово в тайтле» даёт три несовпадения:
#   senior: _GRADE_TO_EXP -> BETWEEN_3_6 -> MIDDLE, а _TITLE_RX -> SENIOR
#   middle: -> BETWEEN_1_3 -> JUNIOR,               а _TITLE_RX -> MIDDLE
#   mid:    -> BETWEEN_1_3 -> JUNIOR,               а _TITLE_RX -> MIDDLE
# Таблицы НЕ выравниваются, потому что отвечают на РАЗНЫЕ вопросы. `_GRADE_TO_EXP` — сколько
# лет опыта требует РАБОТОДАТЕЛЬ (шкала HH; по ней идут срезы аналитики и фильтр
# RESUME_EXP_IDS), и «senior = 3–6 лет» там верно. `_BY_EXP` — какую СВОЮ вилку называть,
# когда известны только годы, и это самооценка владельца профиля, а не рынок. Сдвинуть
# первую — поехали бы аналитика и отбор на шести порталах; сдвинуть вторую — вырастет
# называемая работодателю сумма, а такое решение домен за человека не принимает.
#
# Опасна не сама пара таблиц, а их КОМПОЗИЦИЯ: у не-hh вакансии `experience` получен из
# ярлыка (`Experience.from_grades`), и `from_vacancy` при молчащем тайтле превращает
# «senior» портала в MIDDLE-вилку. Сегодня латентно (Grade потребляют только hh-каналы),
# но любой выход чата или анкет за пределы hh активирует расхождение В ДЕНЬГАХ.
# Набор расхождений зафиксирован стражем `test_grade.py::test_grade_label_roundtrip_*`:
# появится четвёртое — тест упадёт, и решение придётся принять явно.
