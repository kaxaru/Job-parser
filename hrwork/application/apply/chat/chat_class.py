...
from hrwork.config import USER_FIRST_NAME
...
SALARY_Q = re.compile(
    r"вилк|зарплат|заработн\w*\s*плат|оклад|на руки|доход|\bз/?п\b|\bз\.п|сколько хотите"
    r"|ожидани\w*\s+по\s+(?:деньга|доход|оплат|вознагражд)"
    r"|ожидани\w*.{0,25}(зарплат|заработн|оплат|доход|вознагражд)"
    # «Вознаграждение» — только в связке с ОЖИДАНИЕМ или РАЗМЕРОМ, не само по себе: голый
    # корень ловил бы «готовы получать вознаграждение в EUR на volet.com», а это способ и
    # валюта расчёта, а не сумма (закреплено тестом с 27.07). Найдено 20.08.2026 в форм-очереди:
    # «Поделись своими ожиданиями по вознаграждению на старте» уходило мимо детектора, то есть
    # вопрос про деньги пошёл бы к LLM-провайдеру — ровно то, ради чего детектор и заведён.
    r"|вознагражд\w*.{0,25}(ожида|рассчитыва|претенду|хотите)|размер\w*\s*вознагражд"
    ...
    # «От каких сумм рассматриваете предложения?» / «На какую сумму рассматриваете?» — тот же
    # вопрос про вилку, но без слова «зарплата» (живые кейсы 28.07)
    r"|от\s*каких\s*сумм|от\s*какой\s*суммы|минимальн\w*\s*сумм\w*\s*предложен"
    r"|финансов\w*\s*(пожелан|ожидан)|денежн\w*\s*ожидан|на\s*какую\s*сумму"
...
# (живой кейс 27.07). Денежная формулировка остаётся за `уров(ень|н\w*).{0,20}(зарплат|…)`.
#
# ТРЕТЬЯ КОПИЯ УСТРАНЕНА 07.08.2026. Ветки выше пришли из `form_fill._SALARY_Q`, которая
# завелась отдельно вопреки этому же комментарию, и наборы разошлись в ОБЕ стороны:
# только в анкетах ловилось «от каких сумм»/«финансовые пожелания», только в чатах —
# английское `salary`/`compensation`/`day rate`. Английский вопрос про деньги в АНКЕТЕ
# уходил мимо детерминированного пути к LLM, хотя ставка по грейду известна.
PLACE_Q = re.compile(
...
# Обращение по имени владельца («<Имя>, здравствуйте!»): снимается, чтобы персонализированные
# рассылки сравнивались как один шаблон. Имя приходит из профиля (`config.USER_FIRST_NAME`), в
# исходнике его нет (до 25.09.2026 стояло литералом — личные данные в репозитории). Ключа в
# профиле нет -> шаблон, который не матчит ничего: тексты сравниваются как есть, а рассылка с
# обращением по имени опознаётся только задним числом, по повтору среди других чатов.
def name_prefix(name: str) -> re.Pattern[str]:
    """Regex «обращение по имени» для заданного имени; пустое имя — не матчит ничего."""
    if not name.strip():
        return re.compile(r"(?!x)x")
    return re.compile(r"^\s*" + re.escape(name.strip()) + r"\s*[,!.]?\s*", re.I)


_NAME_PREFIX = name_prefix(USER_FIRST_NAME)
