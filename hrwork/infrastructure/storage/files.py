...
def collected_at() -> float | None:
    """Unix-время последнего УДАЧНОГО сбора из `cache_meta.json`; метки нет -> None.

    Отдельная функция, а не ветка `cache_valid`: тот отвечает на «собирать ли сейчас» и по
    любой неожиданности молча возвращает False (что означает «собрать заново» — безопасный
    исход). Здесь исход другой: метка едет в браузер как возраст среза, и «метки нет» обязано
    отличаться от «срез старый». Первое — молчание (ленте нечего сказать), второе — баннер.
    Поэтому None возвращается только при реально нечитаемой метке, а не при любой ошибке.

    `bool` отсекается до `int`: в Python True — это 1, и meta, поправленная руками, дала бы
    «срез собран 01.01.1970», то есть вечный баннер вместо честного «не знаю»."""
    meta: dict[str, Any] = read_json_or(META_FILE, {})
    ts: Any = meta.get('collected_at')
