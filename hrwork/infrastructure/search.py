...
_DESC = "coalesce(description, '')"
SNIPPET_CHARS = 180   # длина обрезки описания в PLAIN-режиме (символы ИСХОДНОГО текста)


def _esc(expr: str) -> str:
    """HTML-экранирование SQL-выражения. Порядок замен важен: `&` первым."""
...
# /api/apply и раздача data/ (аудит 08.08.2026, п.3).
_ESC_DESC = _esc(_DESC)
# Сниппет PLAIN-режима: РЕЖЕМ ДО экранирования, экранируем уже обрезанное (аудит 09.08.2026).
# Обрезка ПОСЛЕ экранирования резала по экранированному тексту и рвала сущность пополам
# (`&amp;` -> `&am`), а сущность попадает в DOM без `esc()` — то есть как есть. Плюс видимая
# длина плавала: 180 символов после экранирования — это от ~140 до 180 символов исходного.
# Безопасность не ослаблена: экранирование по-прежнему на стороне SQL и ДО вставки в DOM,
# а разрубленный пополам тег (`<a hre`) экранируется вместе с остальным текстом.
_ESC_CUT_DESC = _esc(f"left({_DESC}, {SNIPPET_CHARS})")
...
_PLAIN = _Mode(
    source=TABLE,
    rank="NULL::real",
    snippet=f"{_ESC_CUT_DESC} || '…'",   # начало описания -> «…» в конце (обрезка, затем экран.)
