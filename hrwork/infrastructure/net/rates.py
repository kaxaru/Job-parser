...
def to_rub(amount: float | None, currency: str | None,
    ...
    ПУСТАЯ ВАЛЮТА — ТОЖЕ None, а не рубли. До 08.08.2026 здесь стояло `or "RUB"`
    с обоснованием «HH по умолчанию рублёвый»: верное для hh и катастрофическое для
    восьми глобальных порталов, приехавших позже. Замер на срезе 109 596 вакансий:
    вилок без валюты 169, и НИ ОДНОЙ из них нет у hh — это talanto (27) и web3 (142),
    где суммы вида 1300, 10000, 13750 очевидно долларовые. Трактовка их рублями занижала
    вилку примерно в 90 раз: работа за $10 000/мес весила 10 000 ₽, уезжала в самый низ
    сортировки и пряталась любым фильтром «зарплата от» — то есть терялись ровно самые
    дорогие удалённые вакансии в выдаче.

    Валюту НЕ ДОДУМЫВАЕМ и здесь: `web3career.py::_salary` намеренно оставляет её пустой
    (на крипто-рынке платят и в USD, и в стейблкоинах), и подстановка USD «по смыслу»
    была бы той же ошибкой с другим знаком. Нет единицы измерения -> нет сравнимого
    числа; вызывающий обязан обработать None, а не подставить ноль."""
    if amount is None:
        return None
