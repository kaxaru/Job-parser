...
def _salary(it: dict[str, Any]) -> Salary | None:
    ...
    аналитике (`net/rates.py::to_rub` сконвертировал бы по курсу то, что валютой не является;
    метода `Salary.to_rub` не существует — домен в сеть за курсами не ходит).
