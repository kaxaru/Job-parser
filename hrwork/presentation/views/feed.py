...
# ── Чипы опыта ────────────────────────────────────────────────────────────────
#: Пятый чип опыта: у карточки грейда НЕТ. Код — ПУСТАЯ СТРОКА, ровно как в `exp_id`
#: таких записей, поэтому `model.js::filterVacancies` сравнивает его как обычный код
#: и правки не требует.
#:
#: Заведён 10.08.2026 по жалобе владельца: фильтр Python + arbeitnow + опыт давал НОЛЬ
#: вакансий даже при ВСЕХ нажатых кнопках, а без фильтра по опыту — 538. Причина:
#: `f.exps.has(v.exp_id)` не находит пустую строку среди четырёх доменных кодов, поэтому
#: «выбрано всё» переставало быть равносильно «фильтр снят». Задето 27 880 карточек
#: (21,7 % базы): web3 и arbeitnow поголовно, talanto — 22 625 из 59 672, getmatch — 482
#: из 573, hirify — 1644.
#:
#: Грейд НЕ ДОДУМЫВАЕТСЯ до `noExperience`, хотя так было бы короче: у talanto среди
#: безгрейдовых полно senior-вакансий, и подпись «Без опыта» на них была бы прямой
#: неправдой — заодно поехали бы срез по опыту в дашборде и 25 баллов за опыт в скоринге.
