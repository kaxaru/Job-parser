...
**Письмо для крон-откликов** — `cover_template` там же. Подстановки `{name}` (тайтл) и
`{employer}` (компания). Пусто — дефолт из кода. Не путать с `src/feed/cover.js`: тот
для откликов из ленты, этот уходит с автооткликами.

**Ответы на анкеты-опросники** — `answers.form_answers`, список `{q, a}`, где `q` это
РЕГЕКС по тексту поля. По умолчанию **пуст**, и это значит, что движок отвечает только на
вопрос о зарплате (детерминированный код по грейду) и через LLM, а остальные поля уходят
человеку. Наполнять по мере встречи новых вопросов — точную формулировку показывает
`python hh.py forms --dry`, он ничего не отправляет.
...
**Тексты писем из ЛЕНТЫ** — `src/feed/cover.js`: `COVER_TEMPLATES`, `RESUME_TECH_SET`,
`TECH_LABEL`. Письмо крон-откликов настраивается в профиле (`cover_template`, выше).
