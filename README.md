...
arbeitnow.com, himalayas.app, web3.career, themuse.com, jobicy.com, devitjobs.uk плюс
job-борды работодателей на greenhouse.io и ashbyhq.com): сбор -> аналитика ->
...
- **Сбор** — двенадцать источников параллельно, дедуп внутри портала и кросс-портальный.
  Санити-гейт не даёт деградированному срезу затереть кеш. Помимо досок объявлений —
  ATS-борды самих работодателей: там вакансий вчетверо больше, чем те же компании
  выкладывают на агрегаторы.
...
`min(apply-limit, дневной остаток, остаток скользящих 24ч)`: у HH есть и второй, не
документированный потолок — выше ~48 откликов за сутки он перестаёт их подтверждать
(`HH_APPLY_ROLLING_CAP`, `docs/errors.md` 23.09.2026). Поэтому несколько мелких запусков
за день не превысят ни дневной cap, ни окно.
...
**Блеклисты** (`blacklists`) — какие вакансии не брать. Двенадцать ключей: `lead`,
