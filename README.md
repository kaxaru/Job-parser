...
**Блеклисты** (`blacklists`) — какие вакансии не брать. Девять ключей: `senior`,
`management`, `non_engineering`, `qa`, `analyst`, `ml`, `devops`, `other_lang` и
`target_engineering` (последний — исключение, а не запрет: снимает `analyst` и `ml`).
...
Не в репозитории (`.gitignore`, создаются на месте): `data/` — состояние и артефакты сборки,
`logs/`, `personal/` — резюме и заметки, `resume_profile.json`, `.env`.
...
- **Поднятие резюме** — только бесплатная кнопка с гардом по тексту «в поиске»; платную
  «Поднять автоматически» скрипт не трогает.
- **DDoS-Guard** иногда отдаёт headless деградированную страницу: переход переживает челлендж
  за три попытки, а при неудаче пишет в лог, а не молча «0». Если headless стабильно
  упирается — добавить `--headed`.
