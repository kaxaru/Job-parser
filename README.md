...
**Блеклисты** (`blacklists`) — какие вакансии не брать. Десять ключей: `senior`,
`internship`, `management`, `non_engineering`, `qa`, `analyst`, `ml`, `devops`, `other_lang`
и `target_engineering` (последний — исключение, а не запрет: снимает `analyst` и `ml`).
