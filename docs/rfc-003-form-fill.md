...
- **(b) Инъекция → выдумка / неполное извлечение → авто-отправка.** Человек-submit снят поправкой.
  ...
  **Поправка 08.08.2026 (аудит роем, находки 37/45/46).** Completeness-gate проверял полноту
  ЗАКРЫТИЯ полей, но не полноту их ИЗВЛЕЧЕНИЯ: `form_read.py::_tasks`/`_selects` держали один
  `contextlib.suppress` на весь цикл, поэтому исключение на 5-м поле возвращало 4, и гейт
  честно рапортовал «4 из 4 закрыто» — по неполному списку. Теперь suppress на ИТЕРАЦИЮ плюс
  сверка числа `task-body` с числом извлечённых полей; расхождение = не шлём.
  Там же закрыты два соседних: `_sanitize` признавал только ГОЛОЕ `DECLINE` по равенству,
  так что «DECLINE — по фактам резюме ответить нельзя» вписывалось в анкету и уходило
  (единый предикат `_is_decline` по вхождению, как в `answer_quiz`); и для TEXTAREA появился
  ДЕНИЛИСТ — описание вакансии идёт в промпт целиком, это ЧУЖОЙ текст, а `_URL` до этого
  применялся только к не-TEXTAREA. Отправленный текст логируется.
  **Остаточный риск остаётся принятым владельцем** («Авто-submit при полноте»): открытый текст
  может быть неточен и уходит без просмотра — компенсируется грунтовкой, денилистом
  и `--dry`-превью очереди.
- **(c) Чужой URL уводит сессионный браузер.** origin-гард `hh.ru` (Фаза 1); non-persistent контекст (Фаза 2).
- **(d) Утечка резюме в провайдера.** allowlist фактов + `_scrub_pii(resume.md)`: гражданство,
  город, дата рождения, контакты и зарплата не уходят.
  **Поправка 08.08.2026 (находка 44).** Обещание держалось на дырявом скрабе: в `_PII_LINE`
  не было слова «город», «Дата рождения: 12.05.1990» не ловилась ни по «родил», ни цифровым
  шаблоном (точки рвали класс), а `_CTX_KEYS`-поля (`years_text`, `education_text`) шли
  в промпт вообще без скраба. Хуже всего был ЭТАЛОН: `resume_profile.example.json` учил
  писать «Работаю удалённо из города &lt;город&gt;» без pii-флага. Тест при этом был
  тавтологичен — фикстура строилась ПОД сами регексы. Теперь скрабятся и allowlist-поля,
  pii-флаг работает по принципу default-deny, а тест берёт правдоподобные строки резюме
  и сверяет литеральный результат.
