...
(тестов пока нет — см. [`testing.md`](testing.md)).
...
- Статика из `data/`: gzip + ETag для текстовых типов, `304` при совпадении `If-None-Match`
...
- **Занят lock** (идёт крон-батч) -> `queued` + позиция в `apply_pending.json`; крон
  дожмёт очередь в конце своего прогона. Это не ошибка
...
- `fresh` — класс свежести из белого списка: `fresh` (≤30 дн) | `recent` (31–60) | `ghost` (>60);
  пороги из `domain.freshness` (`FRESH_DAYS`/`GHOST_DAYS`)
- `source` — портал из белого списка: `hh` | `hirify` | `talanto`
...
- **`total`** считается через `count(*) OVER()` в том же запросе, отдельного COUNT нет
...
**Назначение.** Файлы из `data/`; `/` -> `feed.html`.
...
**Implementation.** `_Handler._serve_static` -> `_gzip_resp` -> `_gzipped`
...
**Почему так.** Без кэша каждый запрос пере-сжимал бы ~112 МБ `feed-desc.js`.
ETag с `no-cache` даёт `304` вместо повторной передачи, но заставляет браузер
