...
- `core`, `exp_ids` -> `RESUME_CORE` / `RESUME_EXP_IDS` — ядро стека и опыт, общие для
  autoclick-фильтра и скоринга ленты (инжектятся в `feed-data.js`)
...
- `answers` — факты для шаблонных ответов бот-рекрутерам (`chat.md`), включая
  `practices` (ответы про процессы) и `birth_date` (только для `{age}` в анкетах)
...
- `APPLY_EXTRA_EXP_IDS` <- `extra_exp_ids` — `["between3And6"]`, опыт СВЕРХ `exp_ids` только
  для отбора откликов: вилку «3–6 лет» массово ставят на мидл-позиции, а поднимать сам
  `exp_ids` нельзя — поедет процент совпадения в ленте
