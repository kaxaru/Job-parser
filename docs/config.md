...
**`ACCOUNT` и `ACCOUNT_DIR` — аккаунт hh.ru процесса (RFC-004).** Выбирается переменной
окружения `HR_ACCOUNT` при запуске: пусто или `main` — основной, иначе код папки
`data/accounts/<code>/` с `account.json` (`{"label": "...", "login": "..."}`, без телефона и
паролей) и своим `resume_profile.json`. Разрешение строгое (`accounts.py::resolve_account`; VO и
разбор кода — `domain/account.py`): неизвестный код останавливает процесс с подсказкой, папка
сама не создаётся.

- **`login`** в `account.json` — способ входа не-основного аккаунта: `phone` (авто по
  телефону+SMS) или `manual` (ручной ввод в окне, дефолт при отсутствии ключа). Значение
  `password` для не-основного отвергается с ошибкой — это учётка основного (`HH_EMAIL`/
  `HH_PASSWORD`). Основной всегда `password`. Флоу входа — `apply.md`.

- Переменная читается **до** `load_dotenv`: в `.env` она игнорируется, иначе вторым аккаунтом
  стал бы каждый процесс — сервер ленты, сбор, крон основного.
- `ACCOUNT_DIR` — состояние аккаунта (сессия, браузер, квота, журнал, статусы); у `main` это
  `DATA_DIR`. Что общее, а что своё — `storage.md`.
- Профиль резюме у не-основного — `data/accounts/<code>/resume_profile.json`.
- Не-основному открыт только режим `autoclick` (`hh.py::account_mode_error`), анкеты
  выключены независимо от `FORMS_LLM`, автовход по `HH_EMAIL`/`HH_PASSWORD` не выполняется —
  вход по `login` из `account.json` (phone/manual, `apply.md`).
- Под pytest переменная снимается до импорта config (корневой `conftest.py`).
...
- `DEVITJOBS_MAX_TIME = 90` — свой таймаут этого единственного запроса (~4 МБ) вместо
  `CURL_MAX_TIME`: с 25 с он падал на пике этапа 1 сбора. Ретраи и бэкофф — в адаптере.
...
- `STALE_CACHE_HOURS = 36` — с какого возраста среза лента показывает баннер «данные
  устарели» (`COLLECTED_AT_PY`/`STALE_HOURS_PY` -> `model.js::staleAge`). Намеренно БОЛЬШЕ
  `CACHE_TTL_HOURS`: тот отвечает «пора ли собирать», этот — «сбор перестал доезжать до
  кеша», и между ними обязан помещаться один пропущенный прогон плюс запас на сон машины,
  иначе баннер горел бы каждое утро и его перестали бы замечать
...
- `APPLY_EMPLOYER_BLOCKLIST = []` — работодатели, куда не откликаемся ни с какого аккаунта;
  в `.env` через `;`, целое слово, похожие латинские и кириллические буквы не различаются.
  Общий для всех аккаунтов, поэтому в `.env`, а не в профиле резюме (RFC-004, `apply.md`).
  Под pytest всегда пуст: корневой `conftest.py` обнуляет переменную до импорта config
...
- `APPLY_OFFICE_CITIES` <- `office_cities` — дефолт `{Москва, Санкт-Петербург}` (тир 3). Личные
  города (в т.ч. свой) перечисляются в профиле: список ЗАМЕНЯЕТ дефолт целиком (25.09.2026
  город владельца убран из исходника — он раскрывал, где живёт пользователь)
- `APPLY_ACCEPT_HYBRID` <- `accept_hybrid` — брать ли ГИБРИД (`Schedule.HYBRID`, «flexible») в
  ЛЮБОМ городе как удалёнку (RFC-004). Ключа нет -> `False`: у основного гибрид идёт офисной
  веткой (только `APPLY_OFFICE_CITIES`), поведение не меняется. Второй аккаунт ставит `true` —
  гибрид под наш стек в любом городе (частично из дома), пул ниши заметно шире. Влияет ТОЛЬКО
  на `Schedule.HYBRID`; офис (`fullDay`) в чужом городе всё равно мимо
...
- `APPLY_ROLES` <- `apply_roles` — белый список ролей под автоотклик, ярлыки `Role.label`
  (`["Backend", "Разработчик", "GenAI"]`). **Ключа нет -> `None`, роли не ограничены** — так
  живёт основной профиль; сужает отбор второго аккаунта hh.ru (RFC-004). Пустой список —
  не откликаться никуда. Ярлык разбирается строго (`candidates.py::APPLY_ROLE_SET`):
  опечатка валит импорт, а не выключает роль молча
- `APPLY_DEVELOPER_TITLE_WORDS` <- `developer_title_words` — слова (формат blacklists, `*` в
  конце), одно из которых обязано стоять в тайтле вакансии с ролью «Разработчик». Эта роль —
  фолбэк детектора: тайтл роль не назвал, язык нашёлся в описании. Ключа нет -> проверки нет
  (основной профиль). Превью второго аккаунта 15.09.2026: без ключа 8 из 12 вакансий пула были
  такими — «Инженер-разработчик Linux в сетевой ОС», «Marketing Analytics»
...
- `FORMS_ENABLED` (env `FORMS_LLM`) — **по умолчанию OFF**, truthy-set ЯВНЫЙ
  (`1/true/yes/on`); `off`/`n` НЕ включают. Не-основному аккаунту всегда OFF: ответы берутся
  из профиля и резюме основного (RFC-004)
...
- `TECH_PATTERNS` — 61 regex (объём пришпилен `test_prescreen.py`, он же считает пресин).
  14.08.2026 добавлены шесть под профиль владельца: `SQLAlchemy`, `Alembic`, `Celery`,
  `Airflow` (питоновский бэкенд и оркестрация) плюс `LangChain` и `RAG` — прикладной GenAI
  отдельно от общего мешка `ML/AI`, который не различает обучение моделей и работу поверх
  них. Правка словаря меняет `DETECT_SIG` и **сама** протухает кеш техов — пересчёт
  произойдёт на ближайшей загрузке.
  Отдельные хаки задокументированы на месте: `Go` требует
...
- `RESUME_STACK_TIERS` — ярусы стека для скоринга: профиль задаёт СПИСКИ ИМЁН
  (`stack_tiers: {core, adjacent, background}`), веса ярусов (1 / 0.5 / 0.2) знает код —
  человек не должен подбирать числа, чтобы поправить свой стек. Формат словесный, как
  у блеклистов.
- `RESUME_ROLE_FIT` — множитель ЖЕЛАННОСТИ роли (ярлык `Role` -> число). Это отдельная ось
  от стека: ярус отвечает «насколько я готов», множитель — «насколько я хочу». Роль вне
  карты даёт 0.5, а не 0. Полный разбор модели — [`feed.md`](feed.md).
- `RESUME_CORE_SATURATION = 3` — сколько попаданий в ядро считать полной вовлечённостью.
- `RESUME_LANGS` / `RESUME_LANG_FIT` — ось ЯЗЫКА, **первая проверка скоринга**. Мои языки
  выводятся из `RESUME_CORE ∩ LANG_KEYS`, отдельной настройки нет: ядро уже отвечает на
  «на чём я пишу», второй источник разъехался бы. Множители: свой язык 1.0, язык не назван
  0.5, назван только чужой 0.1. Без этой оси «Ведущий разработчик 1С» набирал 91 %
  (`docs/feed.md`).
...
`apply/browser.py`, `apply/cover.py`, `net/proxy.py` и `infrastructure/search.py`; шаблон
...
**Секреты:** `HH_EMAIL`, `HH_PASSWORD` (автовход основного), `HR_LOGIN_PHONE` (телефон для
входа аккаунта `login=phone`, 10 нац. цифр; читает `apply/browser.py`; **только в рантайме**, не
пишется ни в `.env`, ни в `account.json`, ни в лог — в лог идут лишь последние 4 цифры),
`HH_ACCESS_TOKEN`, `HH_CLIENT_ID`, `HH_CLIENT_SECRET`, `WEB3_TOKEN`, `OPEN_ROUTER_API_KEY`,
`ANTHROPIC_API_KEY` (письма `--cover llm`, читает `cover.py::llm_cover`; пусто -> шаблон),
