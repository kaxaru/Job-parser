...
- `SOURCES` — **12 значений**: `hh,hirify,talanto,getmatch,arbeitnow,himalayas,web3,themuse,`
  `jobicy,greenhouse,ashby,devitjobs`.
  Порядок задаёт победителя при кросс-портальном дедупе (`domain/dedup.py`): hh первый, потому
  что с него работает автоотклик. Первые четыре — рынок РФ, остальные восемь — глобальный
  (см. `docs/collect.md`). Неизвестное имя -> warning + пропуск, а не падение.
  **Три последних стоят в конце намеренно:** поставить ATS первыми значило бы переписать
  владельца у ~1900 уже собранных вакансий и сдвинуть все срезы по источникам разом, а
  прирост охвата от порядка не зависит. Менять — отдельным решением и с замером.
- `PORTAL_SITES` — домен портала для подписи в карточке ленты (`hh` -> `hh.ru`, `web3` ->
  `web3.career`, …), 12 записей, по одной на источник. У ATS общего сайта-витрины нет, поэтому
  подпись — имя платформы (`greenhouse.io`, `ashbyhq.com`). Живёт в Python и инжектится в
...
- `GREENHOUSE_BOARDS` (32 слага), `ASHBY_BOARDS` (21 слаг), `ATS_BOARD_CONCURRENCY = 6`,
  `ATS_DEAD_BOARDS_WARN = 0.3` — борды работодателей. **Реестр задаёт охват**: API для
  перечисления компаний у платформ нет, борд доступен только по слагу. Из 74 проверенных
  слагов ответили 53. Список протухает молча, когда компания переезжает между ATS, поэтому
  доля молчащих бордов сравнивается с порогом: ниже — INFO с перечнем, выше — WARNING.
- `DEVITJOBS_URL` — эндпоинт devitjobs.uk. Пагинации нет, весь срез приходит одним запросом
  (2058 записей). Описания в нём нет вовсе — детект стека идёт по `technologies`.
- `GLOBAL_SOURCES_IT_ONLY = 1` — отсев не-IT на входе **восьми** не-РФ источников:
  arbeitnow, himalayas, web3, themuse, jobicy, greenhouse, ashby, devitjobs (это общие
  job-борды: 66 % и 63 % не-IT; борд работодателя — 43 %, там продажи и саппорт лежат
  рядом с разработкой). `0` — забирать всё, включая ритейл и логистику.
...
- `ROLE_PATTERNS` — **17 ключей**, **порядок важен** (первое совпадение). Ключи обязаны
  совпадать со значениями `Role`, иначе `ValueError` на импорте `parsing.py`. В самом `Role`
  членов 18: `NON_IT` — доменный дефолт, регекса под него в карте нет (не-IT отсекает
  `parsing._HARD_NON_IT` и фолбэк). Роли `Support` в карте нет намеренно — поддержка
  отсекается раньше. **`GenAI` обязан стоять раньше `Data/ML`** — порядок и есть граница
  между ними, страж `test_role.py::test_genai_is_matched_before_data_ml`; перестановка
  ключей местами тихо обнулила бы роль.
