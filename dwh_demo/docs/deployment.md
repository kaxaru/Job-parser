...
**Вне крона BI-часть стенда выключена** (с 10.09.2026). Родительский `cron/cron_collect.bat`
поднимает `hh-postgres`, `clickhouse`, `mssql` и `metabase` перед ETL (шаг 0), а последним
шагом останавливает всё, кроме `hh-postgres`, — на нём поиск в ленте родителя. Причина —
память: круглосуточно три сервиса держали ~4,6 ГБ WSL ради часа работы в сутки. Остановка
`stop -t 60`, а не `down`: контейнеры и тома целы, а `unless-stopped` вручную остановленный
сервис сам не поднимает. Таймаут — запас ClickHouse после заливки; MSSQL он не спасает:
`launch_sqlservr.sh` (PID 1 образа) запускает `sqlservr` фоном и SIGTERM не пробрасывает, так
что по таймауту всегда SIGKILL. Это не ново — так MSSQL гасился и при каждом рестарте Docker;
база восстанавливается за 4–9 с, а факт в ней ETL перезаливает целиком. Открыть витрины
между прогонами:

```bash
docker compose up -d --wait clickhouse mssql metabase
```
